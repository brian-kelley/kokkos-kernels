    ...
    static void syr2(const typename AViewType::execution_space& space, const char trans[], const char uplo[],          \
      ...
      if (A_is_ll) {                                                                                                   \
        ...
        KOKKOS_ROCBLAS_SAFE_CALL_IMPL(                                                                                 \
            rocblas_dsyr2(handle, fillMode, N, &alpha, X.data(), one, Y.data(), one, A.data(), LDA));                \
  ...
  struct SYR2<                                                                                                        \
    ...
    static void syr2(const typename AViewType::execution_space& space, const char trans[], const char uplo[],         \
      ...
      if (A_is_ll) {                                                                                                  \
        ...
        KOKKOS_ROCBLAS_SAFE_CALL_IMPL(rocblas_set_stream(handle, space.hip_stream()));                              \
        KOKKOS_ROCBLAS_SAFE_CALL_IMPL(                                                                                \
            rocblas_ssyr2(handle, fillMode, N, &alpha, X.data(), one, Y.data(), one, A.data(), LDA));               \
        KOKKOS_ROCBLAS_SAFE_CALL_IMPL(rocblas_set_stream(handle, NULL));                                            \
      } else {                                                                                                        \
    ...
    static void syr2(const typename AViewType::execution_space& space, const char trans[], const char uplo[],          \
      ...
      if (justTranspose) {                                                                                             \
        if (A_is_ll) {                                                                                                 \
          KokkosBlas::Impl::RocBlasSingleton& s = KokkosBlas::Impl::RocBlasSingleton::singleton();                     \
          KOKKOS_ROCBLAS_SAFE_CALL_IMPL(rocblas_set_stream(handle, space.hip_stream()));                             \
          KOKKOS_ROCBLAS_SAFE_CALL_IMPL(rocblas_zsyr2(handle, fillMode, N,                                           \
        ...
        if (A_is_ll && (alpha.imag() == 0.)) {                                                                         \
          KokkosBlas::Impl::RocBlasSingleton& s = KokkosBlas::Impl::RocBlasSingleton::singleton();                     \
          KOKKOS_ROCBLAS_SAFE_CALL_IMPL(rocblas_set_stream(handle, space.hip_stream()));                             \
          KOKKOS_ROCBLAS_SAFE_CALL_IMPL(rocblas_zher2(handle, fillMode, N,                                           \
                                                      reinterpret_cast<const rocblas_double_complex*>(&alpha),         \
                                                      reinterpret_cast<const rocblas_double_complex*>(X.data()), one,  \
                                                      reinterpret_cast<const rocblas_double_complex*>(Y.data()), one,  \
                                                      reinterpret_cast<rocblas_double_complex*>(A.data()), LDA));      \
    ...
    static void syr2(const typename AViewType::execution_space& space, const char trans[], const char uplo[],          \
      ...
      if (justTranspose) {                                                                                             \
        if (A_is_ll) {                                                                                                 \
          KokkosBlas::Impl::RocBlasSingleton& s = KokkosBlas::Impl::RocBlasSingleton::singleton();                     \
          KOKKOS_ROCBLAS_SAFE_CALL_IMPL(rocblas_set_stream(handle, space.hip_stream()));                             \
          KOKKOS_ROCBLAS_SAFE_CALL_IMPL(rocblas_csyr2(handle, fillMode, N,                                           \
        ...
        if (A_is_ll && (alpha.imag() == 0.)) {                                                                         \
          KokkosBlas::Impl::RocBlasSingleton& s = KokkosBlas::Impl::RocBlasSingleton::singleton();                     \
          KOKKOS_ROCBLAS_SAFE_CALL_IMPL(rocblas_set_stream(handle, space.hip_stream()));                             \
          KOKKOS_ROCBLAS_SAFE_CALL_IMPL(rocblas_cher2(handle, fillMode, N,                                           \
                                                      reinterpret_cast<const rocblas_float_complex*>(&alpha),          \
                                                      reinterpret_cast<const rocblas_float_complex*>(X.data()), one,   \
                                                      reinterpret_cast<const rocblas_float_complex*>(Y.data()), one,   \
                                                      reinterpret_cast<rocblas_float_complex*>(A.data()), LDA));       \
